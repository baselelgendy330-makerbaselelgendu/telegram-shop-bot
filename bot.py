...
# ━━━━━ الإيموجيات الفخمة بناءً على الـ IDs اللي بعتها ━━━━━
EMOJI = {
    "announcement": "5424818078833715060",  # إعلان / تنبيه
    "vip": "6088920147072915408",           # التاج
    "verified": "5976653524476368012",       # الصح / الضمان
    "lightning": "5440621591387980068",      # الساعة / السرعة
    "heart": "5364201435858744869",          # القلب الجديد
    "wallet": "6088990159334808217",         # المحفظة
    "telegram": "6089099509202164251",       # التليجرام
    "arrows_down": "5443038326535759644",    # الرسالة المتحركة / الأسهم
    
    # إيموجيات أساسية مساعدة
    ...
    "success": "5976653524476368012", # مربوط بعلامة الصح
...
# ━━━━━ قاعدة بيانات المنتجات مع الديسكربشن ━━━━━
PRODUCTS = {
    "cdk_chatgpt": {
...
async def init_db():
    ...
    async with db_pool.acquire() as conn:
        await conn.execute("CREATE TABLE IF NOT EXISTS users (telegram_id BIGINT PRIMARY KEY, username TEXT, first_name TEXT, lang TEXT DEFAULT 'ar', balance_usdt NUMERIC DEFAULT 0, created_at TIMESTAMP DEFAULT NOW());")
        ...
        await conn.execute("CREATE TABLE IF NOT EXISTS deposits (id SERIAL PRIMARY KEY, telegram_id BIGINT, method TEXT, amount NUMERIC, currency TEXT, product_key TEXT DEFAULT 'cdk_chatgpt', status TEXT DEFAULT 'pending', created_at TIMESTAMP DEFAULT NOW());")
...
def product_label(product_key: str):
    return PRODUCTS.get(product_key, PRODUCTS["cdk_chatgpt"])["title_en"]
...
# ━━━━━ رسالة تسليم الكود الفخمة جداً ━━━━━
def get_delivery_text(lang: str, product: dict, code: str):
    if lang == "en":
...
def checkout_payment_buttons(lang: str, product_key: str):
    if lang == "en":
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💰 Pay from Wallet", callback_data=f"pay_wallet_{product_key}")],
            [InlineKeyboardButton(text="🟡 Pay via Binance UID", callback_data=f"pay_binance_{product_key}")],
            [InlineKeyboardButton(text="Back", callback_data=f"product_{product_key}")],
        ])
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💰 الدفع من المحفظة", callback_data=f"pay_wallet_{product_key}")],
        [InlineKeyboardButton(text="🟡 الدفع عبر بينانس UID", callback_data=f"pay_binance_{product_key}")],
...
async def wallet_command(message: Message):
    ...
    msg = await animate_message(message, lang)
    await edit_or_answer(msg, f"{ce('wallet','💰')} <b>AIX WALLET</b>\n━━━━━━━━━━━━━━\n\nUSDT Balance: <b>{balance_usdt} USDT</b>" if lang == "en" else f"{ce('wallet','💰')} <b>AIX WALLET</b>\n━━━━━━━━━━━━━━\n\nرصيد الدولار: <b>{balance_usdt} USDT</b>", reply_markup=wallet_kb(lang))
...
async def set_language(call: CallbackQuery):
    ...
    await call.message.answer(f"{ce('verified','✅')} Language changed to English" if lang == "en" else f"{ce('verified','✅')} تم تغيير اللغة للعربية", reply_markup=home_keyboard(lang), parse_mode="HTML")
    await call.answer()
...
async def product_cdk_chatgpt(call: CallbackQuery):
    ...
    caption = (
        ...
        f"━━━━━━━━━━━━━━\n\n"
        f"{ce('wallet','💰')} Price: <b>${product['usd']}</b>\n"
        f"{ce('stock','📦')} Stock: <b>{count}</b>\n"
        f"{ce('shield','🛡')} Subscription: <b>1 Year, no warranty {ce('error','❌')}</b>\n\n"
        f"{ce('announcement','📢')} <b>Instructions:</b>\n"
        f"{desc}\n\n"
        f"{ce('link','🔗')} <b>Redemption Link:</b> http://gpt.ddfafa.com\n\n"
        f"{ce('lightning','⚡')} Instant delivery after payment."
        if lang == "en" else
        f"{ce('chatgpt','🤖')} <b>{product['title_ar']}</b>\n"
        f"━━━━━━━━━━━━━━\n\n"
        f"{ce('wallet','💰')} السعر: <b>{product['usd']}$</b>\n"
        f"{ce('stock','📦')} المتوفر: <b>{count}</b>\n"
        f"{ce('shield','🛡')} الاشتراك: <b>سنه ، no warranty {ce('error','❌')}</b>\n\n"
        f"{ce('announcement','📢')} <b>تعليمات التفعيل:</b>\n"
        f"{desc}\n\n"
...
async def buy_product(call: CallbackQuery):
    ...
    msg = await animate_message(call.message, lang)
    text = (
        f"{ce('payment','💳')} <b>Checkout</b>\n━━━━━━━━━━━━━━\n\n💰 Price: <b>${product['usd']}</b>\n\n{ce('arrows_down','⬇️')} Choose your payment method:"
        if lang == "en" else 
        f"{ce('payment','💳')} <b>إتمام الشراء</b>\n━━━━━━━━━━━━━━\n\n💰 السعر: <b>{product['usd']}$</b>\n\n{ce('arrows_down','⬇️')} اختار طريقة الدفع:"
    )
    await edit_or_answer(msg, text, reply_markup=checkout_payment_buttons(lang, product_key))
    await call.answer()

# ━━━━━ الدفع من المحفظة والتسليم الفوري ━━━━━
@dp.callback_query(F.data.startswith("pay_wallet_"))
async def pay_wallet_product(call: CallbackQuery):
    product_key = call.data.replace("pay_wallet_", "")
    lang = await get_lang(call.from_user.id)
    product = PRODUCTS.get(product_key)
    if not product: return await call.answer("Error", show_alert=True)

    async with db_pool.acquire() as conn:
        balance = await get_wallet_balance(call.from_user.id)
        price = float(product["usd"])

        # لو الرصيد أقل من السعر
        if balance < price:
            text = (
                f"{ce('error','❌')} <b>Insufficient Balance!</b>\n━━━━━━━━━━━━━━\n"
                f"💰 Price: {price} USDT\n"
                f"💵 Your Balance: {balance} USDT\n\n"
                f"Please deposit funds into your wallet to proceed."
                if lang == "en" else 
                f"{ce('error','❌')} <b>رصيدك غير كافٍ!</b>\n━━━━━━━━━━━━━━\n"
                f"💰 سعر المنتج: {price} USDT\n"
                f"💵 رصيدك الحالي: {balance} USDT\n\n"
                f"برجاء عمل إيداع في المحفظة لتتمكن من الشراء."
            )
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="Deposit Now" if lang=="en" else "إيداع الآن", callback_data="home_deposit")],
        ...
        if not item:
            return await call.answer("Out of stock ❌" if lang == "en" else "المخزون نفد ❌", show_alert=True)

        await conn.execute("UPDATE users SET balance_usdt = balance_usdt - $1 WHERE telegram_id=$2", price, call.from_user.id)
        await conn.execute("UPDATE stock SET sold=true WHERE id=$1", item["id"])
        
        deliver_text = get_delivery_text(lang, product, item['item_data'])
        await safe_edit_or_answer(call.message, deliver_text)
        await call.answer("Purchase successful! ✅" if lang=="en" else "تم الشراء بنجاح! ✅")
        await bot.send_message(ADMIN_ID, f"🛒 <b>Auto-Sale via Wallet!</b>\nUser: @{call.from_user.username}\nProduct: {product['title_en']}\nPaid: {price} USDT", parse_mode="HTML")

# ━━━━━ الدفع اليدوي (بينانس) ━━━━━
@dp.callback_query(F.data.startswith("pay_binance_"))
async def pay_binance_product(call: CallbackQuery):
    product_key = call.data.replace("pay_binance_", "")
    lang = await get_lang(call.from_user.id)
    product = PRODUCTS.get(product_key, PRODUCTS["cdk_chatgpt"])
    msg = await animate_message(call.message, lang)
    
    async with db_pool.acquire() as conn:
        dep_id = await conn.fetchval("INSERT INTO deposits (telegram_id, method, amount, currency, product_key) VALUES($1,$2,$3,$4,$5) RETURNING id", call.from_user.id, "binance", product["usd"], "USDT", product_key)
    ...
    text = (
        f"{ce('binance','🟡')} <b>Binance UID:</b> <code>{BINANCE_UID}</code>\n"
        f"💰 <b>Amount:</b> {product['usd']} USDT\n"
        f"📸 Send screenshot proof here.\n"
        f"🧾 Order ID: #{dep_id}"
        if lang == "en" else 
        f"{ce('binance','🟡')} <b>بينانس UID:</b> <code>{BINANCE_UID}</code>\n"
        f"💰 <b>المبلغ:</b> {product['usd']} USDT\n"
        f"📸 ابعت صورة الإثبات هنا.\n"
        f"🧾 رقم الطلب: #{dep_id}"
    )
    await safe_edit_or_answer(msg, text)
    await call.answer()

# ━━━━━ تأكيد الإيداع والشراء اليدوي ━━━━━
...
async def approve(call: CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        if product_key == WALLET_DEPOSIT_KEY:
            ...
            user_msg = f"{ce('verified','✅')} Deposit approved! Added <b>{dep['amount']} USDT</b> to your wallet." if user_lang == "en" else f"{ce('verified','✅')} تم قبول الإيداع! وتم إضافة <b>{dep['amount']} USDT</b> لمحفظتك."
            await bot.send_message(dep["telegram_id"], user_msg, parse_mode="HTML")
        ...
        if not item:
            await safe_edit_or_answer(call.message, "❌ لا يوجد مخزون")
            await bot.send_message(dep["telegram_id"], f"{ce('error','❌')} Out of stock. Contact support." if user_lang == "en" else f"{ce('error','❌')} المخزون نفد، تواصل مع الدعم.", parse_mode="HTML")
            return
            
        await conn.execute("UPDATE stock SET sold=true WHERE id=$1", item["id"])
        await conn.execute("UPDATE deposits SET status='approved' WHERE id=$1", dep_id)
        
        deliver_text = get_delivery_text(user_lang, product, item['item_data'])
        await bot.send_message(dep["telegram_id"], deliver_text, parse_mode="HTML")
        
    await safe_edit_or_answer(call.message, f"✅ Order #{dep_id} Approved & Delivered")
    await call.answer()
...
async def reject(call: CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        if dep:
            ...
            await bot.send_message(dep["telegram_id"], f"{ce('error','❌')} Order rejected." if user_lang == "en" else f"{ce('error','❌')} تم رفض الطلب.", parse_mode="HTML")
...
async def payment_photo(message: Message):
    ...
    await message.answer(f"📤 Sent for review." if lang == "en" else f"📤 تم إرسال الإثبات للمراجعة.")

@dp.callback_query(F.data == "home_support")
async def support_inline(call: CallbackQuery):
    lang = await get_lang(call.from_user.id)
    msg = await animate_message(call.message, lang)
    await edit_or_answer(msg, f"{ce('support','🎧')} Support: {SUPPORT}", reply_markup=back_home_keyboard(lang))
    await call.answer()

@dp.message(F.text.in_(["💬 الدعم", "💬 Support"]))
async def support_message(message: Message):
    await message.answer(f"{ce('support','🎧')} Support: {SUPPORT}", parse_mode="HTML")
...
async def broadcast(message: Message):
    ...
    for user in users:
        try:
            await bot.send_message(user["telegram_id"], f"{ce('announcement','📢')} {esc(text)}", parse_mode="HTML")
            sent += 1
